...
def check_coverage_ai_binary() -> None:
    """
    Checks if the coverage-ai binary exists in the configured path.

    Raises:
        RuntimeError: If the coverage-ai binary is not found in the expected location.
    """
    binary_path = Path(SETTINGS.get("coverage_ai_host_folder"))
    if not binary_path.exists() or not binary_path.is_file():
        raise RuntimeError(
            f"coverage-ai binary not found at {binary_path}. "
            f"Please build the binary using the tests_integration/build_installer.sh script before running tests."
        )
    else:
        logging.info(f"coverage-ai binary found at {binary_path}.")
