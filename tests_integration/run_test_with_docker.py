...
def run_test(test_args: argparse.Namespace) -> None:
    ...
    try:
        ...
        logger.error(f"Invalid coverage-ai arguments: {e}")
        return
...
def execute_test_in_container(container, command: list, exec_env: dict) -> None:
    ...
    This function copies the `coverage-ai` binary to the container and runs the specified
    ...
    Returns:
        None
    """
    copy_file_to_docker_container(container, SETTINGS.coverage_ai_host_folder, SETTINGS.coverage_ai_container_folder)
...
def compose_test_command(test_args: argparse.Namespace) -> list:
    ...
    This function generates a list of command-line arguments for the `coverage-ai` tool
    ...
    Returns:
        list: A list of strings representing the command-line arguments for the `coverage-ai` tool.
    """
    command = [
        "/usr/local/bin/coverage-ai",
        "--source-file-path",
